...
let ascii_tokens () =
  let open Regex in
  let lower = rng 'a' 'z' and upper = rng 'A' 'Z' and digit = rng '0' '9' in
  let alpha = alt lower upper in
  let alnum = alts [ alpha; digit; sc '_' ] in
  [ seq (alt alpha (sc '_')) (star alnum)
  ; plus digit
  ; seqs [ plus digit; sc '.'; star digit; opt (seqs [ of_s "eE"; opt (of_s "+-"); plus digit ]) ]
  ; seqs [ sc '"'
         ; star (alt (chars (Ucharset.diff Ucharset.all ~remove:(Ucharset.of_utf_8_string "\"\\"))) (seq (sc '\\') any))
         ; sc '"' ]
  ; seqs [ str "//"; star (chars (Ucharset.diff Ucharset.all ~remove:(Ucharset.singleton_char '\n'))) ]
  ; plus (of_s " \t\r\n")
  ; alts (List.map str [ "->"; "<-"; ":="; "=="; "!="; "<="; ">="; "&&"; "||"; "|>"; "::" ])
  ; alts (List.map sc [ '('; ')'; '['; ']'; '{'; '}'; ','; ';'; ':'; '.'; '+'; '-'; '*'; '/' ])
  ]
...
let keyword_tokens n =
  ...
  let word () =
    String.init (3 + Random.State.int st 6) (fun _ ->
...
let shape_of workload =
  ...
  let edges = ref 0 and ivals = ref 0 and cedges = ref 0 and civals = ref 0 in
  Dfa.iter_states dfa (fun id ->
    ...
    List.iter
      (fun (cs, d) ->
         let prev = try Hashtbl.find tbl d with Not_found -> [] in
         Hashtbl.replace tbl d (cs :: prev))
      ts;
    Hashtbl.iter
      (fun _ css ->
         incr cedges;
         let u = match css with [ c ] -> c | cs -> Ucharset.union_list cs in
         civals := !civals + Ucharset.num_intervals u)
      tbl);
  Printf.printf
    "  %-14s %8d %8d %8d %10d %10d\n%!"
    workload (Dfa.num_states dfa) !edges !cedges !ivals !civals
;;
...
let () =
  ...
  then measure Sys.argv.(1)
    ...
    let reps = 7 in
    Printf.printf "Dfa.of_tokens / Dfa.minimise (ms, best of %d, fresh process each)\n\n" reps;
    Printf.printf
      "  %-14s %10s %10s %8s %8s %8s\n"
      "workload" "build" "minimise" "states" "min" "edges";
    List.iter
      (fun (name, _) ->
